...
exports.deleteassignedAyaDetailsyId = async (req, res) => {
    try {
        const { id } = req.params;
        const deletedEntry = await CustomerReg.findByIdAndDelete(id);
        if (!deletedEntry) {
...
exports.deleteassignedAyaDetailsyId = async (req, res) => {
    try {
        ...
        if (index < 0 || index >= customer.assignedAyaDetails.length) {
            return res.status(404).json({ status: "Failed", message: "index not found or Invalid Index" })
        }

        customer.assignedAyaDetails.splice(index, 1);

        await customer.save();

        res.status(200).json({ status: "Success", message: "Assign Aya Deleted Successfully" })
